...
function secretValuesFor(runState) {
    return Object.values(runState?.secrets || {}).filter(v => typeof v === 'string' && v);
}
...
function resolveNotificationPolicy(automation, event) {
    ...
    if (!baseline) return { enabled: false, level: 'info', channels: ['inapp'] };
    ...
    // The 'where': bell is always on; 'email' is opt-in (and only delivers
    // when a service Google account is connected — see dispatchRunNotification).
    merged.channels = normalizeChannels(merged.channels);
    return merged;
}

/**
 * Deliver an end-of-run notification across the policy's channels. The
 * in-app bell is the always-present default; 'email' additionally sends a
 * plain-text mail via the configured service account (no-op when email
 * isn't set up). Each channel is isolated so one failing doesn't suppress
 * the other; the whole thing is best-effort (callers wrap in try/catch).
 */
async function dispatchRunNotification(automation, policy, { title, message }) {
    if (!policy || !policy.enabled) return;
    const channels = Array.isArray(policy.channels) && policy.channels.length ? policy.channels : ['inapp'];
    if (channels.includes('inapp')) {
        try {
            ...
            console.warn(`[AutomationRunner] bell notify failed for ${automation.id}: ${e.message}`);
        }
    }
    if (channels.includes('email')) {
        try {
            await sendRunEmail(automation, { subject: title, message });
        } catch (e) {
            console.warn(`[AutomationRunner] email notify failed for ${automation.id}: ${e.message}`);
        }
    }
}

/**
 * Send a run-event email to the automation owner via the platform service
 * account (Gmail-API path in utils/emailService). Silently no-ops when no
 * service email is connected or the owner has no address on file, so an
 * enabled email channel never errors a run when delivery isn't configured.
 */
async function sendRunEmail(automation, { subject, message }) {
    const emailService = require('../../utils/emailService');
    const cfg = await emailService.getServiceEmailConfig().catch(() => null);
    if (!cfg || !cfg.configured) return; // no service email connected — skip quietly
    const userStore = require('../../stores/userStore');
    const user = await userStore.getUser(automation.userId).catch(() => null);
    const to = user?.email;
    if (!to) return;
    const text = `${message || ''}\n\n— Sent by your Bee Flow automation “${automation.title || ''}”.`;
    await emailService.sendServiceEmail({ to, subject, text });
}
    ...
    try {
        ...
        if (user?.organizationId) {
            try {
                ...
                // The org's *active* integration grants live in
                // `org_enabled_integrations` (exposed as orgEnabledIntegrations) —
                // that's what the runtime tool gate (getOrgEnabledIntegrations /
                // entitlements in getIntegrationTools) honours. Reading only the
                // legacy `enabledIntegrations` column here meant orgs that use the
                // current column got an empty list, so buildUserAuth loaded NO
                // OAuth provider tokens and every Google/Microsoft integration was
                // unavailable in routine + dry-run. Union both, then fall back to
                // the global default. (This only decides which provider tokens to
                // load; per-tool access is still enforced downstream by isAppOn.)
                const lists = [];
                if (Array.isArray(org?.orgEnabledIntegrations)) lists.push(...org.orgEnabledIntegrations);
                if (org?.enabledIntegrations) {
                    const legacy = typeof org.enabledIntegrations === 'string'
                        ? JSON.parse(org.enabledIntegrations) : org.enabledIntegrations;
                    if (Array.isArray(legacy)) lists.push(...legacy);
                }
                if (lists.length) {
                    orgEnabledIntegrations = [...new Set(lists)];
                } else {
...
class ApprovalRequiredError extends Error {
    constructor(stepId, prompt, expiresAt = null) {
...
async function execApproval(step, ctx, runState, mode) {
    ...
    // notification with the live link is sent there. The deadline (§WS2.2) is
    // computed here from the step config so the finalize path can persist it.
    const ttlMs = resolveApprovalTtlMs(step);
    const expiresAt = ttlMs ? new Date(Date.now() + ttlMs).toISOString() : null;
    throw new ApprovalRequiredError(step.id, step.prompt || step.title || 'Approval requested', expiresAt);
}
...
async function execForEachStep(step, ctx, runState, mode, runLeaf, checkCancel = null) {
    ...
    if (!Array.isArray(list)) {
        return { output: { iterations: 0, succeeded: 0, failed: 0, results: [], skipped: 'forEach overRef did not resolve to an array' } };
    }
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                // Approval pauses and cancellation are not per-item errors —
                // they must halt the run, not get swallowed into the results.
                if (e instanceof ApprovalRequiredError || e.message === 'Run cancelled') throw e;
                lastErr = e;
    ...
    return { output: { iterations: items.length, succeeded: items.length - failed, failed, results } };
}
...
function resolveArrayRef(step, runState) {
    ...
    if (v.length > cap) {
        const err = new Error(`Collection op input has ${v.length} items (max ${cap}). Add a filter/limit step upstream or raise AUTOMATION_COLLECTION_MAX_ITEMS.`);
...
async function runDag(def, ctx, runStateInit, mode, dispatchStep, { recordSteps = true, branchIndex = null, skipUntilStepId = null, onlyStepId = null, fromStepId = null } = {}) {
    ...
    while (queue.length > 0) {
        ...
        const replaying = stillSkipping && !isTargetForFrom && !isTargetForOnly
            && (hasReplayData || !fillMissingUpstream);
        ...
        if (id === triggerId) {
            ...
            if (recordSteps && ctx.runId && !ctx.stepRecord?.suppress) {
                ctx.emitLifecycle?.('step.started', {
                    stepId: (ctx.stepRecord?.prefix || '') + step.id, stepType: step.type,
                });
            }
            try {
                ...
                //
                // Carve-outs (correctness-critical): an approval pause and a
                // cancellation are NOT step failures — swallowing them into an
                // error branch would auto-"handle" a human-in-the-loop gate or
                // keep a cancelled run walking. Both re-throw unconditionally.
                ...
                if (stepErr instanceof ApprovalRequiredError) throw stepErr;
...
module.exports = { RUNNER_INTERVAL_MS, POLLING_INTERVAL_MS, REAPER_INTERVAL_MS, RETENTION_INTERVAL_MS, MAX_CONCURRENT, MAX_LAYER_DEPTH, RUN_HARD_TIMEOUT_MS, MAX_RUN_TIMEOUT_MS, REAPER_FLOOR_MS, REAPER_BUFFER_MS, REAPER_MAX_ATTEMPTS, APPROVAL_DEFAULT_TTL_MS, APPROVAL_MAX_TTL_MS, COLLECTION_OP_MAX_ITEMS, INSTANCE_ID, resolveApprovalTtlMs, clampRunTimeout, cloneRunValue, secretValuesFor, resolveNotificationPolicy, dispatchRunNotification, sendRunEmail, withConnectorIdentity, resolveUserSession, mergeEnabled, isEmptyToolResult, isToolErrorResult, enrichNextcloudError, buildAdjacency, nextEdgesFor, execIntegrationAction, collectAiStepOutputFields, execAiStep, execCondition, execNotification, execCode, ApprovalRequiredError, execApproval, execParallel, execLoop, buildLinearEdges, execForEachStep, execCallLayer, execLayerOutput, callerCanUseBlock, loadBlockForRun, execCallBlock, execSet, execDateTime, execWait, execStopError, execSwitch, resolveArrayRef, execFilter, execLimit, execDedupe, execAggregate, execSummarize, runDag };
